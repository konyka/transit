...
- `unsubscribe` after a drop drops callbacks and the consumer bit
  (forgets a consumer-only open). A later producer `OPEN` must not
  resurrect the consumer — that `PUSH` would sit inflight with no
  callback.
